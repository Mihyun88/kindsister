...
export default function Dashboard() {
  ...
  const stats = {
    ...
    매매: properties.filter(p => p.prop_type === '매매').length,
      ...
      <main style={{maxWidth:'1100px',margin:'0 auto',padding:'1.2rem 1rem'}}>
        ...
        {isJeonwolse && (
          <>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,minmax(0,1fr))',gap:'10px',marginBottom:'10px'}}>
              {statCard('오피스텔', stats.오피스텔, () => { setFilter(''); setBuildingFilter('오피스텔') }, buildingFilter==='오피스텔')}
              {statCard('원룸', stats.원룸, () => { setFilter(''); setBuildingFilter('원룸') }, buildingFilter==='원룸')}
              <div/><div/>
            </div>
          </>
        )}
          ...
          <div style={{background:'white',borderRadius:'16px',padding:'1.5rem',width:'min(560px,100%)',border:'1px solid #e8e8e0',marginBottom:'40px'}}>
            ...
            {isSangga && (
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px',marginTop:'10px'}}>
                <div><label style={{display:'block',fontSize:'11px',color:'#666',marginBottom:'4px'}}>업종/상호</label><input autoComplete="off" value={form.biz_type||''} onChange={e => setForm({...form,biz_type:e.target.value})} /></div>
                <div><label style={{display:'block',fontSize:'11px',color:'#666',marginBottom:'4px'}}>관리비</label><input autoComplete="off" value={form.mgmt_fee||''} onChange={e => setForm({...form,mgmt_fee:e.target.value})} /></div>
                <div><label style={{display:'block',fontSize:'11px',color:'#666',marginBottom:'4px'}}>권리금</label><input autoComplete="off" value={form.premium||''} onChange={e => setForm({...form,premium:e.target.value})} /></div>
                <div><label style={{display:'block',fontSize:'11px',color:'#666',marginBottom:'4px'}}>평수</label><input autoComplete="off" value={form.area||''} onChange={e => setForm({...form,area:e.target.value})} /></div>
              </div>
            )}
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px',marginTop:'10px'}}>
              <div><label style={{display:'block',fontSize:'11px',color:'#666',marginBottom:'4px'}}>임차인 연락처</label><input autoComplete="off" value={form.tenant_contact||''} onChange={e => setForm({...form,tenant_contact:e.target.value})} placeholder="010-0000-0000" /></div>
              <div><label style={{display:'block',fontSize:'11px',color:'#666',marginBottom:'4px'}}>임대인 연락처</label><input autoComplete="off" value={form.owner_contact||''} onChange={e => setForm({...form,owner_contact:e.target.value})} placeholder="010-0000-0000" /></div>
            </div>
